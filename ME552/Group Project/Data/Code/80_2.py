...
fuelmass = 41.2 * ureg.gram # Measured value
potmeasurement = 99.3 * ureg.gram # Measured value
watermassmeasurement = 590 * ureg.gram # Measured value
ethanolmass = 318 * ureg.gram # Measured value
bottletotal = 396 * ureg.gram # Measured value
hotwatermass = 440 * ureg.gram # Measured value

# Poured volume of fuel in liters
fuelvolume = 50 * ureg.milliliter # Measured value

# Temperature measurements
starttemp = 296.25 * ureg.kelvin # Measured value
tempreading = Q_(75.7, ureg.degF) # Measured value
ambienttemp = tempreading.to(ureg.kelvin) # Convert to kelvin
finaltemp = 365.65 * ureg.kelvin # Measured value

# Add in the pressure measurements of the sampling tube
# Will be input and output in inches of water
pressurediff = 0.455775 * ureg.inch_H2O_39F # Measured value

# Absolute pressure in Pascals
barpressure = 1024. * ureg.millibar # Measured value

# Exhaust gas constituent measurements
# Points 40 to 621 were measured
COlevel = 12.3252 * 1.e-6 * ureg.mol/ureg.mol # Measured value
CO2level = 291.7894 * 1.e-6 * ureg.mol/ureg.mol # Measured value

# Add in burn time, in seconds
minutes = 28. * ureg.minute # Measured value
seconds = 42. * ureg.second # Measured value
...
mfuel = (fuelv * ethrho80).to(ureg.kilogram)
